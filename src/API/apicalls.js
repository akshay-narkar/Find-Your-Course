...
async function getcourses(dispatch1) {
  try {
    const response = await axios.get('https://courses-api-microverse.herokuapp.com/api/v1/courses');
...
async function signup(signUp, history, errordispatch, userdispatch) {
  try {
    const response = await axios.post('https://courses-api-microverse.herokuapp.com/api/v1/auth', {
      name: signUp.name,
      email: signUp.email,
...
async function signin(params, history, errordispatch, userdispatch) {
  try {
    const response = await axios.post('https://courses-api-microverse.herokuapp.com/api/v1/auth/sign_in', {}, {
      headers: params,
    });
...
async function signout(authParams, history) {
  try {
    ...
    await axios.delete('https://courses-api-microverse.herokuapp.com/api/v1/auth/sign_out', { params: authParams });
    history.push('/login');
  } catch (error) {
    // return error;
  }
...
async function getfavs(userid, dispatch1, authParams, errordispatch) {
  try {
    const response = await axios.get(`https://courses-api-microverse.herokuapp.com/api/v1/users/${userid}/favs`,
      { params: authParams });
...
async function addfavapi(userid, dispatch1, authParams, id, errordispatch) {
  try {
    const response = await axios.post(`https://courses-api-microverse.herokuapp.com/api/v1/users/${userid}/favs`,
